...
new #[Layout('layouts.app')] #[Title('Operator Management')] class extends Component
{
    use Toast;
    ...
    public function getOperatorsProperty()
    {
        ...
        return User::where('client_id', $clientId)
            ->whereHas('roles', fn($q) => $q->where('name', 'operator'))
    ...
    public function save(): void
    {
        ...
        if ($this->editId) {
            ...
            $this->success('Operator account created.');
        }
    ...
    public function delete(): void
    {
        ...
        $this->deleteId = null;
        $this->warning('Operator deleted.');
    }

    public function render(): \Illuminate\View\View
    {
        return view('pages.client.users.operator', [
...
<div>
    <x-header title="Operator Management" subtitle="Manage operator accounts for your faculty" separator>
        <x-slot:actions>
            <x-input placeholder="Search..." wire:model.live.debounce="search" icon="o-magnifying-glass" clearable />
            <x-button label="Add Operator" icon="o-plus" wire:click="openCreate" class="btn-primary" />
        </x-slot:actions>
    </x-header>
    ...
    <x-card>
        ...
        ]" :rows="$operators" striped>
            @scope('cell_created_at', $operator)
                {{ $operator->created_at->format('d M Y') }}
            @endscope
            @scope('actions', $operator)
                <div class="flex gap-1">
                    <x-button icon="o-pencil" wire:click="openEdit({{ $operator->id }})"
                        class="btn-ghost btn-xs" tooltip="Edit" />
                    <x-button icon="o-trash" wire:click="confirmDelete({{ $operator->id }})"
        ...
        @if($operators->isEmpty())
            <div class="text-center py-12 text-base-content/40">
                <x-icon name="o-users" class="w-12 h-12 mx-auto mb-2 opacity-30" />
                <p class="text-sm">No operators yet. Click "Add Operator" to create one.</p>
            </div>
        @endif
    </x-card>

    {{-- Create / Edit Modal --}}
    <x-modal wire:model="showModal" :title="$editId ? 'Edit Operator' : 'Add Operator'" separator>
        <div class="space-y-4">
            <x-input label="Full Name" wire:model="name" placeholder="e.g. Budi Santoso" icon="o-user" />
            <x-input label="Email" wire:model="email" type="email" placeholder="operator@example.com" icon="o-envelope" />
    ...
    <x-modal wire:model="showDeleteModal" title="Delete Operator" separator>
