...
<div>
    <x-header title="Operator Management" separator>
        <x-slot:actions>
            <x-input wire:model.live.debounce="search" placeholder="Search..." icon="o-magnifying-glass" class="input-sm" />
            <x-button label="Add Operator" icon="o-plus" wire:click="openCreate" class="btn-primary btn-sm" />
        </x-slot:actions>
    </x-header>
                ...
                @forelse($this->users as $user)
                    <tr class="hover:bg-base-200/50">
                        ...
                        <td>
                            ...
                            <x-button icon="o-trash" wire:click="delete({{ $user->id }})" wire:confirm="Delete this operator?" class="btn-ghost btn-xs text-error" />
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="4" class="text-center py-4 text-base-content/40">No operators yet</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>

    <x-modal wire:model="showModal" title="{{ $editId ? 'Edit' : 'Add' }} Operator">
        <div class="space-y-3">
