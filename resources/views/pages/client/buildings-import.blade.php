...
new class extends Component
{
    ...
    {
        $this->validate(['file' => 'required|file|mimes:csv,xlsx,xls|max:2048']);
...
<div>
    <x-header title="Import Building Data" subtitle="Upload CSV atau XLSX" separator />
        ...
        <div class="bg-base-100 rounded-2xl border border-base-200 shadow p-6">
            <form wire:submit.prevent="import">
                <div class="mb-4">
                    ...
                    <input type="file" wire:model="file" accept=".csv,.xlsx,.xls" class="file-input file-input-bordered w-full" />
                    @error('file') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                </div>
                <div class="mb-4 text-xs text-base-content/50">Format: satu kolom berisi nama gedung per baris.</div>
                <button type="submit" class="btn btn-primary w-full">Import Data</button>
            </form>
            <div class="mt-4 text-center">
                <a href="{{ route('client.buildings') }}" class="text-sm text-primary hover:underline">Lihat semua buildings →</a>
            </div>
        </div>
    </div>
</div>
